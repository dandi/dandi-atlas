...
def main():
    parser = argparse.ArgumentParser(description="Update DEMO-FOV.json from an NWB file.")
    ...
    colors = [
        hex_to_rgb(acronym_to_color.get(r.decode() if isinstance(r, bytes) else r, "888888"))
        for r in br
    ]
    ...
    for label, lx, ly, lz, color in zip(lm_labels, lm_x, lm_y, lm_z, lm_colors):
        ...
        landmarks.append({"label": label, "color": color, "xyz": [float(lx), float(ly), float(lz)]})
